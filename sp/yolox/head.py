"""Adapted from https://github.com/Megvii-BaseDetection/YOLOX/blob/main/yolox/models/yolo_head.py"""
...
from sp.yolox.utils import meshgrid
...
class YOLOXHead(nn.Module):
    ...
    def forward(self, xin):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            inference_output = torch.cat([reg_output, obj_output.sigmoid(), cls_output.sigmoid()], 1)
            inference.append(inference_output)
        ...
        if not self.training:
            self.hw = [x.shape[-2:] for x in inference]
            # [batch, n_anchors_all, 85]
            predictions = torch.cat([x.flatten(start_dim=2) for x in inference], dim=2).permute(0, 2, 1)
            predictions = self.decode_outputs(predictions, dtype=xin[0].type())
