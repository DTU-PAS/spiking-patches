...
class GNNDetector(nn.Module):
    def __init__(self, config: Config):
        ...
        self.final_pool = GridPool(config)

    def forward(self, data: Batch, batch_size: int):
        ...
        x = self.final_pool(x, data_pooled.pos, data_pooled.batch, batch_size=batch_size)
        return x
...
class GridPool(torch.nn.Module):
    def __init__(self, config: Config):
        super().__init__()
        self.hidden_size = config.gnn.dim3

        height, width = load_dimensions(config.dataset)
        grid_size = config.gnn.grid_size
    ...
    def forward(self, x: torch.Tensor, pos: torch.Tensor, batch: torch.Tensor, batch_size: int) -> torch.Tensor:
