...
use crate::continuous_spiking_patch::ContinuousSpikingPatch;
...
pub struct Tokenizer {
    ...
    patch_size: usize,
    patches: Vec<ContinuousSpikingPatch>,
    spike_threshold: f64,
}
...
impl Tokenizer {
    pub fn new(
        ...
        assert!(0.0 < relative_refractory_scale);
        assert!(relative_refractory_scale < 1.0);

        // sum absolute and refractory here to automatically account for absolute refractory period when checkng for refractory period
        let relative_refractory_period = absolute_refractory_period + relative_refractory_period;

        // initialise spiking patches
        ...
        let mut patches: Vec<ContinuousSpikingPatch> = Vec::with_capacity(num_patches);
        (0..num_patches).for_each(|i| {
            let x = i as u16 % num_patch_columns_u16;
            let y = i as u16 / num_patch_columns_u16;

            let patch = ContinuousSpikingPatch::new(
    ...
    pub fn tokenize(&mut self, x: Location, y: Location, t: Time, p: Polarity) -> Tokens {
        ...
        let floored_threshold = self.spike_threshold.floor() as usize;
        let max_tokens = num_events / floored_threshold;
